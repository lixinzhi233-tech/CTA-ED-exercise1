...
```{r}
#if add another column based on orginal form
evdes %>%
  group_by(year) %>%
  mutate(count = n())
...
ggplot(evtsperyr) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA))#expand: leave no space limits: scale of y axis
```
...
```{r}
tidy_des %>%
  count(word, sort = TRUE)#a new dataframe
```
...
tidy_des <- tidy_des %>%
  filter(!word %in% remove_reg)#words not in remove_reg
```
...
edbf_term_counts <- tidy_des %>% 
  group_by(year) %>%
  count(word, sort = TRUE)#build a new dataframe to count each words according to different years
```
...
```{r}
edbf_term_counts$womword <- as.integer(grepl("women|feminist|feminism|gender|harassment|sexism|sexist|girl|girls|female", #true false
                                            x = edbf_term_counts$word))#recognize word in counts if include things above
#turn the resulte t and f as int(01)
```
...
edbf_counts <- edbf_term_counts %>%
  group_by(year) %>%
  mutate(year_total = sum(n)) %>% #count all words' numb by year
  filter(womword==1) %>% # then filter 
  summarise(sum_wom = sum(n),
            year_total= min(year_total)) #build a new one 
```
...
```{r}
df_clean = edbfdata %>% select(year,description)
#filter token remove stopwords
total = df_clean %>% mutate(obs = 1 ) %>% group_by(year) %>% summarize(totalnumber = sum(obs)) #cnt of total num of each yr
...
tidy_des = tidy_des %>% filter(!word %in% stop_words$word)
...
tidy_count = tidy_des %>% group_by(year) %>% count(word,sort = TRUE)

family_words <- c(
  "family", "household", "home", "domestic",'couple',
  "spouse", "husband", "wife", "partner", "marriage", 
  "child", "children", "son", "daughter", "kid", "offspring", 
  "parent", "mother", "father", "mom", "dad", 
  "sibling", "brother", "sister", 
  "grandparent", "grandmother", "grandfather", 
  "relative", "relatives", "family member"
)


individual_words <- c(
  "self", "ego", "identity", "autonomy", "independence", "individual", "person", "people",
  "someone", "somebody", "anyone", "anybody", 'self','ego',
  "choice", "decision", "preference", "initiative", "freedom", "liberty", "uniqueness",
  "self-esteem", "self-worth", "introspection", "reflection", "experience", "perception",
  "belief", "opinion", "motivation", "desire", "ambition", "aspiration"
)

# mark family
tidy_count$family_flag <- as.integer(
  grepl(paste(family_words, collapse = "|"), x = tidy_count$word, ignore.case = TRUE)
)

# mark self
tidy_count$individual_flag <- as.integer(
  grepl(paste(individual_words, collapse = "|"), x = tidy_count$word, ignore.case = TRUE)
)
```

```{r}
tidy_count_home <- tidy_count %>%
  group_by(year) %>%
  mutate(year_total = sum(n)) %>%
  filter(family_flag==1) %>%
  summarise(sum_home = sum(n),
            year_total= min(year_total))
#all key wors related to 'family' count
```

```{r}
tidy_count_person = tidy_count %>% group_by(year) %>% mutate(year_total1 = sum(n)) %>% 
  filter(individual_flag ==1)%>% 
  summarise(sum_person = sum(n),year_total = min(year_total1))
#all related to individualism
```

```{r}
ggplot(tidy_count_home, aes(year, sum_home / year_total, group=1)) +
  geom_line() +
  xlab("Year") +
  ylab("% family-related words") +
  geom_smooth(method = lm, se = FALSE)+
  scale_y_continuous(labels = scales::percent_format(),
                     expand = c(0, 0), limits = c(0, NA)) +
  theme_tufte(base_family = "Helvetica") 

ggplot(tidy_count_home,aes(year,sum_home)) + geom_line() + xlab("year") + ylab("total amount of family-related words") +scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_tufte(base_family = "Helvetica") 

```
```{r}
ggplot(tidy_count_person,aes(year,sum_person / year_total,group = 1)) + geom_line() + 
  xlab('year') + ylab('% individual-related words') + geom_smooth(method = lm, se = FALSE) + scale_y_continuous(labels = scales::percent_format(),expand = c(0, 0), limits = c(0, NA)) + theme_tufte(base_family = "Helvetica") 

ggplot(tidy_count_person,aes(year,sum_person)) + geom_line() + xlab('year') + ylab('total amount individual-related words') + 
  scale_y_continuous(expand = c(0,0),limits = c(0,NA)) + theme_tufte(base_family = "Helvetica" )
...
# merge and frequency 
tidy_count_percent <- tidy_count_home %>%
  select(year, sum_home, year_total) %>%
  mutate(category = "Family", freq = sum_home / year_total) %>%
  bind_rows(
    tidy_count_person %>%
      select(year, sum_person, year_total) %>%
      mutate(category = "Individual", freq = sum_person / year_total)
  )

# merge 
tidy_count_total <- tidy_count_home %>%
  select(year, sum_home) %>%
  mutate(category = "Family", total = sum_home) %>%
  bind_rows(
    tidy_count_person %>%
      select(year, sum_person) %>%
      mutate(category = "Individual", total = sum_person)
  )
...
```{r}
ggplot(tidy_count_percent, aes(x = year, y = freq, color = category, group = category)) +
  geom_line(size = 1) +
  geom_smooth(method = "lm",  se = FALSE,linetype = "dashed") +   # 回归曲线虚线
  scale_color_manual(values = c("Family" = "#1b9e77", "Individual" = "#d95f02")) +  # 柔和配色
  scale_y_continuous(labels = scales::percent_format(), expand = c(0, 0), limits = c(0, NA)) +
  labs(
    x = "Year",
    y = "% of words",
    color = "Category",
    title = "Comparison Between Family and Individual Words"
  ) +
  theme_tufte(base_family = "Helvetica") +
...
```
```{r}
ggplot(tidy_count_total, aes(x = year, y = total, color = category, group = category)) +
  geom_line(size = 1) +
  geom_smooth(method = "lm", se = FALSE,linetype = 'dashed') +
  scale_color_manual(values = c("Family" = "skyblue", "Individual" = "#F5DEB3")) +
